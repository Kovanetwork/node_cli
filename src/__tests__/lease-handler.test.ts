...
describe('LeaseHandler', () => {
  ...
  beforeEach(() => {
    ...
    handler = new LeaseHandler(config, mockExecutor as any, mockP2p as any);
  });
  ...
  it('should start and immediately poll for leases', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      'http://localhost:3000/api/v1/provider/leases',
  ...
  it('should flush logs periodically', async () => {
    ...
    const logCallback = mockExecutor.on.mock.calls.find(
      (c: any[]) => c[0] === 'log'
    )![1];
